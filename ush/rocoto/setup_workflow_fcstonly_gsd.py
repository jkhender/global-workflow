...
taskplan = ['getic', 'init', 'waveinit', 'waveprep', 'fcst', 'post', 'wavepostsbs', 'wavepostbndpnt', 'wavepostpnt', 'wavegempak', 'waveawipsbulls', 'waveawipsgridded', 'wafs', 'wafsgrib2', 'wafsblending', 'wafsgcip', 'wafsgrib20p25', 'wafsblending0p25', 'postsnd', 'gempak', 'awips', 'vrfy', 'metp', 'arch']
...
def get_resources(dict_configs, cdump='gdas'):
    ...
    for task in taskplan:
        ...
        if scheduler in ['slurm'] and machine in ['JET'] and task in ['fcst']:
            strings.append('\t<!ENTITY PARTITION_%s "vjet,sjet">\n' % taskstr )
        if scheduler in ['slurm'] and machine in ['JET'] and task in ['post']:
            strings.append('\t<!ENTITY PARTITION_%s "vjet,xjet">\n' % taskstr )
...
def get_workflow(dict_configs, cdump='gdas'):
    ...
    if do_wave in ['Y', 'YES'] and do_wave_cdump in ['GFS', 'BOTH']:
        ...
        deps.append(rocoto.add_dependency(dep_dict))
        dep_dict = {'type': 'task', 'name': '%sinit' % cdump}
        deps.append(rocoto.add_dependency(dep_dict))
        dependencies = rocoto.create_dependency(dep_condition='and', dep=deps)
