...
!--- In (physics only)
    ...
    real (kind=kind_phys), pointer :: aod_in (:)   => null()  !< total atmospheric extention (550 nm) aod input
...
! Output - only in physics
    ...
    real (kind=kind_phys), pointer :: dcli3dt (:,:,:) => null()  !< moisture change due to physics
...
!--- In/Out
    ...
    if (Model%ldiag3d) then
      ...
      if (Model%qdiag3d) then
        allocate (Diag%dq3dt  (IM,Model%levs,14))
        allocate (Diag%dcli3dt  (IM,Model%levs,6))
      else
        allocate (Diag%dq3dt  (1,1,13))
      endif
    else
      allocate (Diag%du3dt  (1,1,9))
      allocate (Diag%dv3dt  (1,1,9))
      allocate (Diag%dt3dt  (1,1,12))
      allocate (Diag%dq3dt  (1,1,14))
    endif
