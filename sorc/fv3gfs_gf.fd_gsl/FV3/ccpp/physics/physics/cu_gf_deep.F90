...
!
      ...
      do i=its,itf
         if(ierr(i).eq.0)then
           ...
           enddo
!    smooth dellas between kts+1 and ktop -1 and replace updated t,q,h values
           do k=kts+1,ktop(i)-1
            xt(i,k)= .25*(dellat(i,k-1)+dellat(i,k+1)+2.*dellat(i,k))*mbdt+tn(i,k)
            xq(i,k)= .25*(dellaq(i,k-1)+dellaq(i,k+1)+2.*dellaq(i,k))*mbdt+tn(i,k)
            xhe(i,k)= .25*(dellah(i,k-1)+dellah(i,k+1)+2.*dellah(i,k))*mbdt+tn(i,k)
            xt(i,k)=max(190.,xt(i,k))
           enddo
         endif
      enddo
...
!---------------evap below cloud base
      ...
      do i=its,itf
          if(ierr(i).eq.0 .and.pre(i).gt.0.) then
             ...
             enddo
             do k=kts,ktop(i)
               !outt(i,k)=outt(i,k)-sig(i)*xlv/cp*outqc(i,k)
               !outqc(i,k)=outqc(i,k)*(1.-sig(i))
             enddo
...
!
               ...
               if(name == "deep" )then
                    clwdet=0.02                  ! 05/27/2021
                    kklev=maxloc(zu(i,:),1)      ! 05/05/2021
                    if(k.lt.kklev) clwdet=0.     ! 05/05/2021
               else
                    clwdet=0.02                  ! 05/27/2021
                    if(k.lt.kklev) clwdet=0.     ! 05/25/2021
               endif
